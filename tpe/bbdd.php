...
function showProducto($id_producto){
        ...
        $idcatprod = $producto[0]->id_categoriaFK;

        $query = $db->prepare("SELECT * FROM categoria WHERE id_categoria = $idcatprod");
        $query->execute();
        $categoria = $query->fetchAll(PDO::FETCH_OBJ);

        echo '<h3>Nombre: </h3><h4> '.$producto[0]->nombre.'</h4>';
        echo '<h3>Categoria a la que pertenece: </h3><h4> '.$categoria[0]->nombre.'</h4>';
